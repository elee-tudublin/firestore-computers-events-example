...
async function getAllEvents() {
  ...
  let eventsArray = eventDocs.map(async doc => {
    ...
    // return a new object for this event 
    return new Event(doc.id,
      event.type,
      event.level,
      event.timeStamp,
      // the computer name (using this to replace the id value)
      computer,
      event.service,
      event.user,
      event.description
...
async function getEventsByComputerId(compId) {
  ...
  // return the array after resoling any outstanding promises from async calls
  return await Promise.all(eventsArray);
}